...
class ExcelConditionApplier:
    ...
    def apply_conditions(self, destination_sheet, stock_sheet, builder_id, floor_no):
            try:
    ...
    def apply_first_free_cell_conditions(self, destination_sheet, stock_sheet, builder_id, floor_no):
        try:
            builder_conditions = self.conditions_df[self.conditions_df['Builder Code'].astype(str).str.strip() == str(builder_id).strip()]
            logging.info(f"🔍 Checking First Free Cell Rows for Builder {builder_id}: {builder_conditions.shape[0]} rows (before filtering)")
            ...
            for idx, row in builder_conditions.iterrows():
                ...
                logging.info(f"🧪 Checking if '{sheet_obj.name}' == '野縁'? {'YES' if sheet_obj.name == '野縁' else 'NO'}")
    ...
    def _apply_action(self, sheet, stock_sheet, cell, action):
        try:
            if action.startswith('Overwrite'):
                ...
                logging.info(f"🌇 Set C21")

            elif cell == 'C22':
                sheet[cell].value = '吊元セット～【L＝150】'
                logging.info(f"🌇 Set C22")

            elif cell == 'J12':
                sheet[cell].value = 2
                logging.info(f"🔈 Set J12=2")
