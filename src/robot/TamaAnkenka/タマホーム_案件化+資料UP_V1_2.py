...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("タマホーム_案件化+資料UP_bot.log", encoding="utf-8"), logging.StreamHandler()],
)
...
def mail_dealer():
    ...
    WebDriverWait(driver, 15).until(EC.frame_to_be_available_and_switch_to_it((By.ID, "ifmSide")))
    logging.info("Switched to side Frame")
    time.sleep(1)

    # Wait for and click the "専用アドレス・タマホーム" element
    WebDriverWait(driver, 15).until(
        EC.element_to_be_clickable((By.XPATH, '//span[contains(text(), "専用アドレス・タマホーム")]'))
    ).click()
    logging.info("専用アドレス・タマホーム clicked")
    time.sleep(3)
    # Switch back to the default content and then to the main frame
    driver.switch_to.default_content()
    WebDriverWait(driver, 15).until(EC.frame_to_be_available_and_switch_to_it((By.ID, "ifmMain")))
...
def genba_login():
    ...
    time.sleep(1)
    genba_code = WebDriverWait(driver, 15).until(
        EC.presence_of_element_located(
            (By.XPATH, "/html/body/div[2]/div/div/div[1]/div[3]/form/table/tbody/tr[1]/td/div/input")
        )
    )
    time.sleep(1)
    genba_code.clear()
    ...
    genba_ID = WebDriverWait(driver, 15).until(
        EC.element_to_be_clickable(
            (By.XPATH, "/html/body/div[2]/div/div/div[1]/div[3]/form/table/tbody/tr[2]/td/div/input")
        )
    )
    genba_ID.clear()
    ...
    (
        ...
        .until(
            EC.element_to_be_clickable(
                (By.XPATH, "/html/body/div[2]/div/div/div[1]/div[3]/form/table/tbody/tr[5]/td/button")
            )
        )
        .click()
    )
...
def move_to_another_folder():
    ...
    try:
        ...
        # If NoSuchFrameException is raised, we are already in the iframe
        logging.info("Already in ifmMain iframe")
    shinchaku = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//div[contains(text(), '新着')]"))
    )
...
headings = {"A": "案件番号", "B": "案件名", "C": "住所", "D": "現場番号", "E": "結果"}

# Write the headings to the first row of the first sheet
for col_index, (col_letter, heading) in enumerate(headings.items(), start=1):  # start=1 for 1-based indexing # noqa
...
while total_mails > 0:
    ...
    try:
        ...
        try:
            shinki_level = WebDriverWait(driver, 15).until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        "//td[contains(@class, 'olv-c-table__tbody-td') and count(.//span)=1 and .//span[text() = '新規案件']]",  # noqa
                    )
                )
            )
            shinki_level.click()
            ...
            try:
                sakuzumachi_lavel = WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable(
                        (
                            By.XPATH,
                            "//td[contains(@class, 'olv-c-table__tbody-td') and count(.//span)=1 and .//span[text() = '作図まち']]",  # noqa
                        )
                    )
                )
                ...
                try:
                    ...
                    WebDriverWait(driver, 20).until(
                        EC.presence_of_element_located(
                            (By.XPATH, "//div/span/button[@class='olv-p-mail-view-header__ops-bulk']")
                        )
                    ).click()
                    logging.info("blue tick clicked")
                    time.sleep(1)

                    side_frame = WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located(
                            (By.XPATH, "/html/body/div/div/form/div[2]/section/div[1]/div[2]/div[1]/div[2]/div")
                        )
                    )
                    ...
                    bangou = WebDriverWait(driver, 20).until(
                        EC.visibility_of_element_located(
                            (
                                By.XPATH,
                                "/html/body/div/div/form/div[2]/section/div[1]/div[2]/div[1]/div[2]/div/div[2]/ul/li[5]/div/div[2]/div[1]/div[2]/span",
                            )
                        )
                    )
                    案件番号 = bangou.text
                    ...
                    ankken_mei = WebDriverWait(driver, 20).until(
                        EC.visibility_of_element_located(
                            (
                                By.XPATH,
                                "/html/body/div/div/form/div[2]/section/div[1]/div[2]/div[1]/div[2]/div/div[2]/ul/li[5]/div/div[2]/div[3]/div[2]/span",
                            )
                        )
                    )
                ...
                logging.info("No mail for process")
                break
    ...
    mail_body = (
        WebDriverWait(driver, 15)
        .until(EC.presence_of_element_located((By.XPATH, "//div[@class='olv-p-mail-view-body']/pre")))
        .text
    )
    time.sleep(1)
    if shinki_level:
    ...
    open_url = WebDriverWait(driver, 20).until(
        EC.element_to_be_clickable((By.XPATH, '//*[@id="form-olv-p-viewmail"]/div[2]/section/div/pre/a[1]'))
    )
    open_url.click()
    ...
    try:
        ...
        elements = driver.find_elements(
            By.XPATH, "//span[@class='attachmentList']/span[@class='imageFrame attachmentFile']/a"
        )
        ...
        for element in elements:
            ...
            popup = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((By.XPATH, "//*[@id='dynagonImagePopup-popup']"))
            )
            logging.info("Popup opened.")
            time.sleep(1)

            download_Btn = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.XPATH, '//*[@id="dynagonImagePopup"]/div[2]/div[1]/div[4]/div[1]/a'))
            )
            ...
            logging.info("Closed the new tab and switched back to the original tab.")
            time.sleep(1)
            popup_frame = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located((By.XPATH, "//*[@id='dynagonImagePopup-popup']"))
            )
            time.sleep(1)
            close_Btn = (
                WebDriverWait(driver, 10)
                .until(EC.element_to_be_clickable((By.XPATH, '//*[@id="dynagonImagePopup"]/a')))
                .click()
            )
            time.sleep(1)  # Adjust the delay as needed
            chat_close_btn_pupup = (
                WebDriverWait(driver, 10)
                .until(EC.element_to_be_clickable((By.XPATH, '//span[@class="closeButton"]')))
                .click()
            )
            time.sleep(1)
    except Exception:
    ...
    element_genba = WebDriverWait(driver, 15).until(
        EC.element_to_be_clickable((By.XPATH, '//a/div[contains(text(),"現場情報")]'))
    )
    element_genba.click()
    logging.info("現場情報 ckicked")

    try:
        現場住所 = WebDriverWait(driver, 15).until(
            EC.element_to_be_clickable((By.XPATH, "//span[@class='jobItemAddress']"))
        )
        現場住所_tex = 現場住所.text
    ...
    try:
        genba_bangou = (
            WebDriverWait(driver, 15)
            .until(EC.element_to_be_clickable((By.XPATH, "//span[@class='cell content jobItemNumber']")))
            .text
        )
    ...
    if shinki_level:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable((By.XPATH, '//*[@id="notice-balloon-balloon_display_hdd"]/div[2]/a'))
            ).click()
            logging.info("Pop-up closed")
        except Exception:
            logging.info("No pop-up found or pop-up close button not found within the timeout period")

        three_line = driver.find_element(
            By.XPATH, "//button[@class='icon-button olv-p-gnav__icon' and @title='その他']"
        )
        ...
        WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="wrap"]/table/tbody/tr/td[2]/table[2]/tbody/tr[1]/td/table/tbody/tr/td[5]/a')
            )
        ).click()
        ...
        anken_search = WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable(
                (
                    By.XPATH,
                    "/html/body/div/table/tbody/tr/td[2]/table[1]/tbody/tr/td/table/tbody/tr/td/form/nobr/span[1]/input",
                )
            )
        )
        time.sleep(3)
        ...
        WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable(
                (
                    By.XPATH,
                    '//*[@id="wrap"]/table/tbody/tr/td[2]/table[1]/tbody/tr/td/table/tbody/tr/td/form/nobr/button',
                )
            )
        ).click()
        try:
            ...
            driver.find_element(
                By.XPATH,
                '//*[@id="wrap"]/table/tbody/tr/td[2]/table[2]/tbody/tr[2]/td/div/table/tbody/tr/td/table/tbody/tr/td/div',
            )
        ...
        WebDriverWait(driver, 30).until(
            EC.element_to_be_clickable((By.XPATH, '//*[@id="matterForm"]/div[1]/table/tbody/tr[1]/td/input'))
        ).send_keys("タマホーム")
        # SEND ANKEN MEI
        ...
        date_select = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(
                (By.XPATH, "//nobr[text()='受付日時(入力不要)']/following::a[contains(@href, '__setCurrentDate')][1]")
            )
        )
    ...
    shiryou_path = os.path.join(download_directory, rf"{案件番号} {bukken}", "資料")
    ...
    for file in files:
        ...
        logging.info("The file in the chat has been moved to Shiryou")
    ...
    element_shryou = WebDriverWait(driver, 15).until(
        EC.element_to_be_clickable((By.XPATH, '//a/div[contains(text(),"図面")]'))
    )
    ...
    try:
        ...
        try:
            ...
            file_not_found = WebDriverWait(driver, 15).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//div[@class='message' and text()='ダウンロードできるファイルがありません']")
                )
            )
        ...
        cancel_btn = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable(
                (
                    By.XPATH,
                    "//span[@class='fileMultiSelectModeOffButton button' and .//span[@class='value' and text()='キャンセル']]",  # noqa
                )
            )
        )
        ...
        try:
            WebDriverWait(driver, 5).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//span[@class='subDirectoryName overflow' and text()='住宅仕様確認書']")
                )
            )
            driver.refresh()
            ...
            logging.info("file download form another folder")
            ...
            try:
                ...
                waiting = WebDriverWait(driver, 180).until(
                    EC.presence_of_element_located(
                        (By.XPATH, "//div[@class='barPercent' and contains(text(), '100%')]")
                    )
                )
                logging.info("ZIPファイルをダウンロードしました")
                ...
                WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable(
                        (By.XPATH, "//span[@class='subDirectoryName overflow' and text()='1つ上に戻る']")
                    )
                ).click()
                time.sleep(2)
            ...
            logging.info("no other folder for download")
            # Wait for the download process
            cancel_btn = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (
        ...
        try:
            WebDriverWait(driver, 5).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//span[@class='subDirectoryName overflow' and text()='確定設備図（エプコ）']")
                )
            )
            driver.refresh()
            another_folder = WebDriverWait(driver, 10).until(
            ...
            logging.info("no any other folder for download")

        for root, dirs, files in os.walk(abc):  # noqa
            for file in files:
        ...
        for root, dirs, files in os.walk(abc, topdown=False):  # noqa
            for name in dirs:
    ...
    if shinki_level:
        try:
            ...
            # click on kanrenzuke
            kanrenzuke = WebDriverWait(driver, 10).until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        "//div[contains(@class, 'olv-p-matter-search__ops')]//button[normalize-space(text())='関連付ける']",  # noqa
                    )
                )
            )
            kanrenzuke.click()
    ...
    try:
        ...
        level_btn = (
            WebDriverWait(driver, 10)
            .until(EC.element_to_be_clickable((By.XPATH, "//div[@class='dropdown__trigger']")))
            .click()
        )
        logging.info("Clicked on level selection btn")
        time.sleep(0.5)

        select_frame = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//ul[@class='list has-scroll']"))
        )
        ...
        if shinki_level:
            作図まち_select = (
                WebDriverWait(driver, 10)
                .until(EC.presence_of_element_located((By.XPATH, "//li[text()='作図まち']")))
                .click()
            )
            ...
            大至急_select = (
                WebDriverWait(driver, 10)
                .until(EC.presence_of_element_located((By.XPATH, "//li[text()='大至急']")))
                .click()
            )
