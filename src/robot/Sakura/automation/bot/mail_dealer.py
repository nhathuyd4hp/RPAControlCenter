...
class MailDealer(IBot):
    ...
    @retry_if_exception(failure_return=False)
    ...
    def send_mail(
        ...
        mode: Literal["送信確認", "承認依頼確認", "一時保存"] = "一時保存",
    ) -> bool:
        ...
        while self.browser.execute_script("return document.readyState") != "complete":
            time.sleep(self.retry_interval)
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//button[text()='次へ']"))).click()
        # From
        fr_input = self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[name='fFrom']")))
        fr_input.clear()
        fr_input.send_keys(fr)
        # To
        to_input = self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[name='fTo[]']")))
        to_input.clear()
        to_input.send_keys(to)
        # Subject
        subject_input = self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[id='fSubject']")))
        subject_input.clear()
        ...
        self.browser.execute_script("arguments[0].innerText = arguments[1];", body, content)
        ...
        for attachment in attachments:
            ...
            file_uploader: WindowSpecification = wait_until_passes(
                ...
                func=lambda: Application().connect(title="Open").window(title="Open"),
            )
            wait_until_passes(
                timeout=self.timeout,
                retry_interval=self.retry_interval,
                func=lambda dialog=file_uploader: dialog.child_window(class_name="ComboBoxEx32")
            ...
            uploader_itembox = self.wait.until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, "div[class='md-uploader-itembox']"))
            )
            items = uploader_itembox.find_elements(By.XPATH, "./div")
            items = [item.text.strip() for item in items]
            if os.path.basename(attachment) not in items:
